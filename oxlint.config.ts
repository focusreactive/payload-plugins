...
// ── Architectural import boundaries for payload-plugin-analytics ──────────
// Ported from the package's former eslint.config.js. Restriction fragments
// are shared so each region can restate exactly what applies to it.
...
const SERVER_ONLY_NEXT = {
  group: ["next/*"],
  message: "services/ and endpoints/ are server-only — no next/* imports.",
};
const TEST_SEAM = {
...
export default defineConfig({
  ...
  rules: {
    ...
    // Large component files use forward refs intentionally.
  ...
  overrides: [
    ...
    {
      files: [`${ANALYTICS}/src/**/*.ts`, `${ANALYTICS}/src/**/*.tsx`],
      rules: {
        "no-restricted-imports": ["error", { paths: [GA4_SDK], patterns: [TEST_SEAM] }],
      },
    },
    ...
    //     GA4 + test-seam restrictions from (1) still apply, so restate them.
    {
      ...
      rules: {
        "no-restricted-imports": [
          "error",
          {
            paths: [GA4_SDK, ...SERVER_ONLY_PATHS],
            patterns: [TEST_SEAM, SERVER_ONLY_NEXT],
          },
        ],
      },
    },
    // (3) The two GA4-owning service dirs MAY import the GA4 SDK. They stay
    //     server-only and still must not import the test seam.
    {
      files: [
        `${ANALYTICS}/src/services/ga4DataClient/**/*.ts`,
        `${ANALYTICS}/src/services/ga4DataClient/**/*.tsx`,
        `${ANALYTICS}/src/services/analyticsService/**/*.ts`,
        `${ANALYTICS}/src/services/analyticsService/**/*.tsx`,
      ],
      rules: {
        "no-restricted-imports": [
          "error",
