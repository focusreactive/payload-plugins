...
const jsxConverters: JSXConvertersFunction<NodeTypes> = ({
  ...
  blocks: {
    ...
    logosInline: ({ node }: { node: any }) => (
      <LogosInlineComponent {...(node.fields as any)} />
    ),
  ...
  upload: ({ node }) => {
    ...
    const media =
      typeof uploadNode.value === "object" ? (uploadNode.value as Media) : null;
...
export const RichText = ({
  ...
  return (
    <RichTextReact
      className={cn(
        "prose prose-sm sm:prose-base md:prose-lg max-w-full",
        className
      )}
