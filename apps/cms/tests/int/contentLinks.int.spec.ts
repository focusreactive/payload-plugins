...
import { collectLinkRefs, fetchLinkDocs, isLinkValue, linkRefKey, linkToContentNode } from "@/core/lib/contentExtraction";
import type { LinkResolveCtx, LinkValue, ResolvedLinkDoc } from "@/core/lib/contentExtraction";
...
describe("collectLinkRefs", () => {
  it("collects unpopulated reference ids anywhere in the tree, deduped", () => {
    const values = {
      blocks: [
        ...
        { items: [{ link: { type: "reference", reference: { relationTo: "posts", value: 7 }, label: "b" } }] },
        { actions: [{ type: "reference", reference: { relationTo: "page", value: 1 }, label: "dup" }] },
      ],
    };
  ...
  it("ignores custom, customPage, and already-populated references", () => {
    const values = {
      ...
      c: { type: "reference", reference: { relationTo: "page", value: { slug: "about" } }, label: "r" },
    };
    expect(collectLinkRefs(values)).toEqual([]);
  });
});
...
describe("linkToContentNode", () => {
  it("resolves a custom url", () => {
    ...
    expect(linkToContentNode(link, ctx())).toEqual({ type: "link", href: "https://example.com", text: "Site" });
  });
  ...
  it("resolves a page reference from the fetched docs map (relative, locale-prefixed)", () => {
    const link: LinkValue = { type: "reference", reference: { relationTo: "page", value: 1 }, label: "About" };
    const docs = ctx([[linkRefKey({ collection: "page", id: 1 }), { slug: "about", breadcrumbs: [{ url: "/about" }] }]], "es");
    expect(linkToContentNode(link, docs)).toEqual({ type: "link", href: "/es/about", text: "About" });
  });
  it("resolves a posts reference using the blog basePath", () => {
    const link: LinkValue = { type: "reference", reference: { relationTo: "posts", value: 7 }, label: "Post" };
  ...
  it("uses a populated reference object without a fetched doc", () => {
    const link: LinkValue = { type: "reference", reference: { relationTo: "page", value: { slug: "home", breadcrumbs: [{ url: "/home" }] } }, label: "Home" };
    expect(linkToContentNode(link, ctx())).toEqual({ type: "link", href: "/", text: "Home" });
  });
  it("drops a reference that cannot be resolved", () => {
    const link: LinkValue = { type: "reference", reference: { relationTo: "page", value: 99 }, label: "Gone" };
...
describe("fetchLinkDocs", () => {
  ...
  it("returns an empty map when apiRoute is missing or there are no refs", async () => {
    ...
    expect((await fetchLinkDocs([{ collection: "page", id: 1 }], { apiRoute: undefined, locale: "en" })).size).toBe(0);
  });
  ...
  it("swallows fetch failures and returns an empty map", async () => {
    ...
    const out = await fetchLinkDocs([{ collection: "page", id: 1 }], { apiRoute: "/api", locale: "en" });
