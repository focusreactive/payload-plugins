  ...
  ],
  {
    ...
    variants: {
      backgroundColor: {
        ...
        dark: ["bg-[var(--color-deep-900)] text-slate-50 border-white/10", "hover:shadow-[0_24px_50px_-24px_rgba(0,0,0,0.6)]"],
        "dark-gray": ["bg-[var(--color-pine-900)] text-slate-50 border-white/10", "hover:shadow-[0_24px_50px_-24px_rgba(0,0,0,0.6)]"],
        "gradient-2": ["bg-gradient-to-br from-teal-600 to-ink-950 text-white border-transparent", "hover:shadow-[0_24px_50px_-24px_rgba(0,0,0,0.6)]"],
        none: [
...
const iconTileVariants = cva("relative z-[1] flex size-[46px] shrink-0 items-center justify-center rounded-md transition-transform duration-[360ms] ease-out group-hover:scale-[1.06]", {
  defaultVariants: { backgroundColor: "light" },
  variants: {
    backgroundColor: {
...
const linkVariants = cva("relative z-[1] inline-flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-[0.18em] transition-colors", {
  defaultVariants: { backgroundColor: "light" },
  variants: {
    backgroundColor: {
...
export default function DefaultCard({ image: _image, link, title, description, backgroundColor, icon, rounded, alignVariant }: IDefaultCardProps) {
  const bg = backgroundColor ?? "light";

  const alignClass = alignVariant === "center" ? "items-center text-center" : alignVariant === "right" ? "items-end text-right" : "items-start";
  ...
  return (
    <GlowCard className={cn(cardVariants({ backgroundColor: bg }), roundedClass, alignClass)}>
      {icon !== undefined && <div className={iconTileVariants({ backgroundColor: bg })}>{icon !== null ? icon : <span className="font-display text-lg italic leading-none">✦</span>}</div>}
