...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(
    sql`ALTER TABLE \`presets_blocks_hero\` DROP COLUMN \`description\`;`
  );
...
export async function down({
  ...
  await db.run(
    sql`ALTER TABLE \`presets_blocks_hero\` ADD \`title\` text NOT NULL DEFAULT '';`
  );
  await db.run(
    sql`ALTER TABLE \`presets_blocks_hero\` ADD \`description\` text;`
  );
  await db.run(
    sql`ALTER TABLE \`presets_blocks_copy\` ADD \`text\` text NOT NULL DEFAULT '';`
  );
