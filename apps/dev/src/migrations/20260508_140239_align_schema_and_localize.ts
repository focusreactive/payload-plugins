...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  `);
  await db.run(sql`CREATE UNIQUE INDEX \`presets_locales_locale_parent_id_unique\` ON \`presets_locales\` (\`_locale\`,\`_parent_id\`);`);
  ...
  await db.run(sql`ALTER TABLE \`pages_blocks_copy\` ADD \`_locale\` text NOT NULL;`);
  ...
  await db.run(sql`ALTER TABLE \`_pages_v_blocks_copy\` ADD \`_locale\` text NOT NULL;`);
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`_pages_v_blocks_copy\` DROP COLUMN \`_locale\`;`);
