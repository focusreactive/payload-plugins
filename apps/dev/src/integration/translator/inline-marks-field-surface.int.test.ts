...
/**
 * The per-field translate control, `POST /translate/field`, with container-granular rich text on.
 *
...
describe("per-field translation, the mode on", () => {
  ...
  it("returns the paragraph in the order the reply came back", async () => {
    // Order and formats together: the order proves the reply moved the pieces, the formats prove
    // each one took its own formatting along. Per-node translation can produce neither.
