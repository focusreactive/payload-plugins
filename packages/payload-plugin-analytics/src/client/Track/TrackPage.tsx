...
type TrackPageProps = {
  ...
  | { pageRef: string; collection?: never; id?: never }
  | { collection: string; id: string | number; pageRef?: never }
);
...
export function TrackPage(props: TrackPageProps) {
  ...
  const pageRef =
    props.pageRef === undefined ? formatPageRef(props.collection, props.id) : props.pageRef;
  ...
  useEffect(() => {
    ...
    const resolvedPath =
      ...
      (typeof window === "undefined"
        ? ""
        : getPagePath(window.location.pathname, window.location.search));
