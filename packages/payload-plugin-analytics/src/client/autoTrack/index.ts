...
const BUILT_INS: BuiltInListener[] = [
  ...
  { type: "directions_click", install: installDirectionsClick, flagKey: "directionsClicks", defaultFlag: true },
  { type: "whatsapp_click", install: installWhatsappClick, flagKey: "whatsappClicks", defaultFlag: true },
  { type: "telegram_click", install: installTelegramClick, flagKey: "telegramClicks", defaultFlag: true },
  { type: "form_submit", install: installFormSubmit, flagKey: "formSubmits", defaultFlag: false },
];

export function installLeadActionListeners(provider: AnalyticsProvider, options: AutoTrackLeadActionsConfig | undefined = {}, types: readonly string[] = []): () => void {
