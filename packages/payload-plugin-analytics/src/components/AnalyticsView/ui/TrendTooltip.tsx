...
export function TrendTooltip({ active, payload, metric }: RechartsTooltipProps & { metric: TrendMetric }) {
  ...
  return (
    <div className="bg-[var(--theme-elevation-0)] border border-[var(--theme-border-color)] rounded-[var(--style-radius-s)] px-2.5 py-2 text-xs shadow-popover">
      <div className="text-[11px] text-[var(--theme-elevation-500)] font-[family-name:var(--font-mono)] mb-1">{formatShortDate(date)} 2026</div>
      ...
      {previous != null && (
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span>Previous</span>
          <span className="ml-auto text-[var(--theme-elevation-500)] tabular-nums">{formatMetricValue(metric, previous)}</span>
        </div>
      )}
    </div>
  );
