...
function errorToMessage(err: Error) {
  ...
  if (msg.includes("429") || msg.includes("quota"))
    return "Analytics quota exceeded. Try again in a few minutes.";
...
export function ErrorTile({ error, onRetry, className }: ErrorTileProps) {
  return (
    <div
      className={cn(
        "flex flex-col items-center justify-center py-6 px-3 text-center gap-2",
        className
      )}
    >
      <AlertCircle size={20} className="text-[var(--theme-error-500)]" />

      <div className="text-sm font-medium text-[var(--theme-elevation-800)]">
