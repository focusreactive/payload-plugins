...
export function ChainList({ rows, loading, error, onRetry }: ChainListProps) {
  ...
  return (
    <div className="flex flex-col">
      {rows.map((row, i) => (
        <div key={i} className="grid grid-cols-[24px_1fr_70px_60px] items-center gap-3 py-2.5 px-1 border-b border-[var(--theme-elevation-100)] last:border-b-0">
          <span className="font-[family-name:var(--font-mono)] text-[11px] text-[var(--theme-elevation-500)] text-right">{(i + 1).toString().padStart(2, "0")}</span>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 [scrollbar-width:thin] [-webkit-mask-image:linear-gradient(to_right,black_95%,transparent)] [mask-image:linear-gradient(to_right,black_95%,transparent)]">
            {row.path.map(({ kind, value }, rowIndex) => (
              <Fragment key={rowIndex}>
                ...
                {rowIndex < row.path.length - 1 && <ChevronRight size={12} className="text-[var(--theme-elevation-300)] shrink-0" />}
              </Fragment>
            ))}
          </div>

          <span className="text-right tabular-nums font-semibold text-[13px]">{row.count}</span>
          <span className="text-right text-[var(--theme-elevation-500)] tabular-nums text-[11.5px]">{formatPercentage(row.conversionRate)}</span>
        </div>
      ))}
    </div>
  );
