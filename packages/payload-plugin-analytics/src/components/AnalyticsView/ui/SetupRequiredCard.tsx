...
function renderInlineCode(line: string) {
  return line.split("`").map((seg, i) =>
    i % 2 ? (
      <code key={i} className="font-[family-name:var(--font-mono)] bg-[var(--theme-warning-100)] px-1.5 rounded text-[11px]">
        {seg}
      </code>
    ) : (
      <span key={i}>{seg}</span>
  ...
  return (
    <div className="border border-dashed border-[var(--theme-warning-500)] bg-[var(--theme-warning-50)] rounded-[var(--style-radius-m)] p-5">
      ...
      <div className="text-[var(--theme-warning-700)] text-sm leading-relaxed">
        ...
        <a href={SETUP_GUIDE_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-medium text-[var(--theme-warning-700)] mt-2">
          Setup guide <ExternalLink size={12} />
        </a>
        <p className="mt-2 text-[12.5px] text-[var(--theme-warning-700)]/80">After registering, GA4 takes up to 24 hours to backfill data.</p>
      </div>
    </div>
  );
}
