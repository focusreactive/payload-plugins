...
export function PerPageBreakdownBlock({ dateRange, comparison, className }: BlockComponentProps) {
  ...
  const prevByPagePath = new Map((data?.comparison?.perPage ?? []).map((p) => [p.pagePath, p.counts]));
  ...
  return (
    <DataCard title="Per-page breakdown" icon={FileText} className={className}>
      {isLoading || error ? (
        ...
        <LeadActionsPerPageTable rows={data?.current.perPage ?? []} prevByPagePath={showCompare ? prevByPagePath : undefined} />
      )}
    </DataCard>
  );
}
