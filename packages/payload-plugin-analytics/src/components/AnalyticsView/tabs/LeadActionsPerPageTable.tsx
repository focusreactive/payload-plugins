...
export function LeadActionsPerPageTable({ rows, prevByPagePath }: LeadActionsPerPageTableProps) {
  ...
  return (
    <table className="w-full border-collapse text-[12.5px]">
      <thead>
        <tr>
          <th className="text-left p-2 text-[10px] tracking-widest uppercase text-[var(--theme-elevation-500)] font-semibold border-b border-[var(--theme-border-color)]">Page</th>
          <th className="text-right p-2 text-[10px] tracking-widest uppercase text-[var(--theme-elevation-500)] font-semibold border-b border-[var(--theme-border-color)]">Leads</th>
          <th className="text-left p-2 text-[10px] tracking-widest uppercase text-[var(--theme-elevation-500)] font-semibold border-b border-[var(--theme-border-color)]">Top actions</th>
      ...
      <tbody>
        {rows.map((r, i) => {
          ...
          return (
            <Fragment key={i}>
              <tr onClick={() => setOpen((o) => ({ ...o, [r.pagePath]: !o[r.pagePath] }))} className="cursor-pointer hover:bg-[var(--theme-elevation-50)]">
                <td className="p-2 border-b border-[var(--theme-elevation-100)] font-[family-name:var(--font-mono)] text-xs">{r.pagePath}</td>
                ...
                <td className="p-2 border-b border-[var(--theme-elevation-100)]">
                  <div className="flex gap-2 flex-wrap">
                    {top3.map(([kind, count]) => {
                      ...
                      return <Metric key={kind} value={count} prevValue={prevCount} format={formatNumber} icon={Icon} mode="chip" />;
                    })}
                  </div>
                </td>

                <td className="p-2 border-b border-[var(--theme-elevation-100)] text-right">
                  <ChevronRight size={14} className={cn("text-[var(--theme-elevation-500)] transition-transform", isOpen && "rotate-90")} />
                </td>
              </tr>

              {isOpen && (
                <tr>
                  <td colSpan={4} className="bg-[var(--theme-elevation-50)] p-4 border-b border-[var(--theme-elevation-100)]">
                    <BarList
