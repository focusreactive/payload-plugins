...
export function SessionDrawer({ row, detail, onClose }: SessionDrawerProps) {
  const devices =
    row.deviceCategory.length > 0 ? Array.from(new Set(row.deviceCategory)) : ["other" as const];
  const fullSetupRequired = detail?.setupRequired && detail.missing?.includes("fr_session_id");
  const seqOnlyMissing =
    detail?.missing?.includes("fr_event_seq") && !detail?.missing?.includes("fr_session_id");
