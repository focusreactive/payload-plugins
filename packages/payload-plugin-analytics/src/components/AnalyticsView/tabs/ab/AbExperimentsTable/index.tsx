...
export function AbExperimentsTable({
  ...
  onOpen: (manifestKey: string) => void;
}) {
  return (
      ...
      <table className="w-full border-collapse [&_.ab-updated]:font-mono [&_.ab-updated]:text-[11px] [&_.ab-updated]:text-(--theme-elevation-500) [&_.num]:text-right [&_.num]:tabular-nums [&_td:first-child]:pl-4 [&_td:last-child]:pr-4 [&_td]:border-b [&_td]:border-(--theme-elevation-100) [&_td]:px-3 [&_td]:py-[11px] [&_td]:text-[13px] [&_td]:text-(--theme-elevation-900) [&_tbody_tr:hover]:bg-(--theme-elevation-50) [&_tbody_tr]:cursor-pointer [&_thead_th]:border-b [&_thead_th]:border-(--theme-border-color) [&_th:first-child]:pl-4 [&_th:last-child]:pr-4 [&_th]:px-3 [&_th]:py-[11px] [&_thead_th]:text-left [&_thead_th]:text-[11px] [&_thead_th]:font-semibold [&_thead_th]:text-(--theme-elevation-500)">
              ...
              <Tooltip
                ...
                content={
                  <>
                    ...
                    <TooltipText>
                      Largest relative lift in lead-conversion rate among the variants versus the
                      control bucket. An arrow appears once the result clears statistical
                      significance — grey with no arrow means not yet significant.
                    </TooltipText>
                  </>
                }
              >
                <span className="underline decoration-dotted decoration-(--theme-elevation-300) underline-offset-2">
                  Best conversion lift
                </span>
              </Tooltip>
            </th>
            <th style={{ width: 90 }}>
              <Tooltip
                ...
                content={
                  <>
                    ...
                    <TooltipText>
                      Chi-square test that the observed traffic split matches the configured split.
                      A mismatch points to a bucketing or tracking bug that invalidates results.
                    </TooltipText>
                    <TooltipLegend>
        ...
        <tbody>
          {rows.map((row) => (
            <tr
              key={row.manifestKey}
              onClick={() => onOpen(row.manifestKey)}
              className={cn(experimentRowVariants({ srmFailed: !row.srmPassed }))}
            >
              <td>
              ...
              <td>
                <div className="flex flex-col gap-px">
                  <span className="font-mono text-[12px] text-(--theme-elevation-900)">
                    {row.manifestKey}
                  </span>
                  {row.parentTitle && (
                    <span className="truncate text-[11px] text-(--theme-elevation-500)">
                      {row.parentTitle}
                    </span>
                  )}
                </div>
