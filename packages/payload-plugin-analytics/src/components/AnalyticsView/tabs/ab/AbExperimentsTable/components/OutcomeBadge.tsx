...
import { outcomeBadgeVariants } from "../variants";
import type { OutcomeBadgeTone } from "../variants";
...
export function OutcomeBadge({ outcome }: { outcome: AbExperimentOutcome }) {
  ...
  return (
    <span className={cn(outcomeBadgeVariants({ tone: m.tone }))}>
      {m.tone === "live" ? <span className="h-[7px] w-[7px] rounded-full bg-(--theme-success-500) shadow-[0_0_0_3px_var(--theme-success-100)]" /> : <Icon size={11} />}
      {m.label}
    </span>
  );
}
