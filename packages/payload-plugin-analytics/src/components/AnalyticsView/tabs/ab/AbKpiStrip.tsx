...
function Card({
  ...
  return (
    <div
      ...
      <div
        className={cn(
          "inline-flex items-center gap-1.5 text-xs text-(--theme-elevation-600)",
          tone === "warn" ? "[&_svg]:text-(--theme-warning-700)" : "[&_svg]:text-(--theme-elevation-500)",
        )}>
        <Icon size={13} />
        <span>{label}</span>
      </div>
      ...
      <div
        className={cn(
          ...
          tone === "warn" ? "text-(--theme-warning-700)"
          : tone === "ok" ? "text-(--theme-success-700) dark:text-(--theme-success-500)"
...
export function AbKpiStrip({ data }: { data?: AbKpisResponse }) {
  const { activeExperiments, avgAgeDays, exposedSessions, leadConversions, needingAttention, variantsLive, winRate } =
    data ?? {};
  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-3">
      <Card
        ...
        sub={
          winRate ? `${winRate.winners}/${winRate.qualified} qualified · ${winRate.notQualified} not yet` : undefined
        }
      />
      ...
      <Card
        ...
        value={leadConversions ? formatNumber(leadConversions) : "—"}
        sub="on experiment pages"
      />

      <Card
        icon={Clock}
        label="Avg experiment age"
        value={avgAgeDays ? `${avgAgeDays}d` : "—"}
        sub="median · now − started"
      />
