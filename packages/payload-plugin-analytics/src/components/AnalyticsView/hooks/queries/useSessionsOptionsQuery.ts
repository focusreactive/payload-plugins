...
export function useSessionsOptionsQuery(dateRange: DateRange) {
  return useQuery({
    ...
    queryFn: ({ signal }) => analyticsFetch<SessionsListQuery, SessionsResponse>(ANALYTICS_ENDPOINT_PATHS.sessions, { dateRange, comparison: { kind: "none" }, limit: OPTIONS_LIMIT }, { signal }),
