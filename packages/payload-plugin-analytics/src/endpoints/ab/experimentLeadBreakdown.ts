...
import { shapeLeadBreakdown } from "../../services/queries/shapeAbPanels";
import type { AnalyticsPluginConfig } from "../../types/config";

export function buildAbExperimentLeadBreakdownEndpoint(config: AnalyticsPluginConfig): Endpoint {
  return {
    path: ANALYTICS_ENDPOINT_PATHS.abExperimentLeadBreakdown,
    method: "post",
    handler: withAccess(config, async (req) => {
      ...
      try {
        ...
        return Response.json(shapeLeadBreakdown(stats));
      } catch (err) {
        const gate = abSetupGate(err);
        if (gate) return Response.json({ buckets: [], rows: [], missing: gate.missing });
