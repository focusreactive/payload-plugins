...
export function validateResolvedLayout(layout: AnalyticsLayoutConfig, registry: Record<BlockId, BlockDefinition>): void {
    ...
    for (const [rowId, row] of rows) {
      ...
      for (const [blockId, block] of enabledBlocks) {
        ...
        if (blockOrders.has(block.order)) {
          err(`Tab "${tabId}" row "${rowId}": duplicate block order ${block.order} between "${blockOrders.get(block.order)}" and "${blockId}"`);
        }
