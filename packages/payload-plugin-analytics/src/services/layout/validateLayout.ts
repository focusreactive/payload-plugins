  ...
  for (const [tabId, tab] of Object.entries(layout.tabs)) {
    ...
    for (const [rowId, row] of rows) {
      if (!isPositiveInt(row.columns)) {
        err(
          `Tab "${tabId}" row "${rowId}": columns must be a positive integer, got ${row.columns}`
        );
      }

      if (rowOrders.has(row.order)) {
        err(
          `Tab "${tabId}": duplicate row order ${row.order} between "${rowOrders.get(row.order)}" and "${rowId}"`
        );
      }
      ...
      for (const [blockId, block] of enabledBlocks) {
        ...
        if (!isPositiveInt(block.colSpan)) {
          if (typeof block.colSpan !== "number" || !Number.isFinite(block.colSpan)) {
            err(
              `Tab "${tabId}" row "${rowId}" block "${blockId}": colSpan must be a positive integer`
            );
          }
          if (block.colSpan <= 0) {
            err(
              `Tab "${tabId}" row "${rowId}" block "${blockId}": colSpan must be positive, got ${block.colSpan}`
            );
          }
          if (!Number.isInteger(block.colSpan)) {
            err(
              `Tab "${tabId}" row "${rowId}" block "${blockId}": colSpan must be an integer, got ${block.colSpan}`
            );
          }
        }
        ...
        if (previousRow) {
          err(
            `Tab "${tabId}": block "${blockId}" appears in multiple rows ("${previousRow}", "${rowId}")`
          );
        }
