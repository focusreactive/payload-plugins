...
export function inverseStandardNormalCdf(probability: number): number {
  ...
  const centralNumeratorCoeffs = [
    -3.969683028665376e1, 2.209460984245205e2, -2.759285104469687e2, 1.38357751867269e2,
    -3.066479806614716e1, 2.506628277459239,
  ];
  const centralDenominatorCoeffs = [
    -5.447609879822406e1, 1.615858368580409e2, -1.556989798598866e2, 6.680131188771972e1,
    -1.328068155288572e1,
  ];
  const tailNumeratorCoeffs = [
    -7.784894002430293e-3, -3.223964580411365e-1, -2.400758277161838, -2.549732539343734,
    4.374664141464968, 2.938163982698783,
  ];
  const tailDenominatorCoeffs = [
  ...
  if (probability < lowerTailBreakpoint) {
    ...
    return (
      (((((tailNumeratorCoeffs[0]! * tailVar + tailNumeratorCoeffs[1]!) * tailVar +
        tailNumeratorCoeffs[2]!) *
        tailVar +
        tailNumeratorCoeffs[3]!) *
        tailVar +
        tailNumeratorCoeffs[4]!) *
        tailVar +
        tailNumeratorCoeffs[5]!) /
      ((((tailDenominatorCoeffs[0]! * tailVar + tailDenominatorCoeffs[1]!) * tailVar +
        tailDenominatorCoeffs[2]!) *
        tailVar +
        tailDenominatorCoeffs[3]!) *
        tailVar +
        1)
    );
    ...
    return (
      ((((((centralNumeratorCoeffs[0]! * centeredProbabilitySquared + centralNumeratorCoeffs[1]!) *
        centeredProbabilitySquared +
        centralNumeratorCoeffs[2]!) *
        centeredProbabilitySquared +
        ...
        centeredProbability) /
      (((((centralDenominatorCoeffs[0]! * centeredProbabilitySquared +
        centralDenominatorCoeffs[1]!) *
        centeredProbabilitySquared +
        centralDenominatorCoeffs[2]!) *
        centeredProbabilitySquared +
...
export function minimumDetectableEffect(
  ...
  alpha = 0.05,
  power = 0.8
): MinimumDetectableEffectResult | null {
