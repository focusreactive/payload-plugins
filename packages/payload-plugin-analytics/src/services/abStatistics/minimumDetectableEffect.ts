...
export function inverseStandardNormalCdf(probability: number): number {
  ...
  if (probability < lowerTailBreakpoint) {
    ...
    return (
      (((((tailNumeratorCoeffs[0]! * tailVar + tailNumeratorCoeffs[1]!) * tailVar + tailNumeratorCoeffs[2]!) * tailVar
        + tailNumeratorCoeffs[3]!)
        * tailVar
        + tailNumeratorCoeffs[4]!)
        * tailVar
        + tailNumeratorCoeffs[5]!)
      / ((((tailDenominatorCoeffs[0]! * tailVar + tailDenominatorCoeffs[1]!) * tailVar + tailDenominatorCoeffs[2]!)
        * tailVar
    ...
    return (
      ((((((centralNumeratorCoeffs[0]! * centeredProbabilitySquared + centralNumeratorCoeffs[1]!)
        ...
        * centeredProbability)
      / (((((centralDenominatorCoeffs[0]! * centeredProbabilitySquared + centralDenominatorCoeffs[1]!)
        * centeredProbabilitySquared
