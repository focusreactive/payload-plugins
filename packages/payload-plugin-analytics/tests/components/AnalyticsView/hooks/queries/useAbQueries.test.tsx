...
import { useAbKpisQuery, useAbExperimentExposureQuery } from "../../../../../src/components/AnalyticsView/hooks/queries/useAbQueries";
...
describe("useAbKpisQuery", () => {
  it("posts to /api/analytics/ab/kpis and resolves data", async () => {
    const payload = { activeExperiments: 2, variantsLive: 3, exposedSessions: 100, leadConversions: 10, avgAgeDays: 12, needingAttention: 0 };
...
describe("useAbExperimentExposureQuery", () => {
  it("is disabled when manifestKey is null and does not fetch", async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ buckets: [], srmPassed: true, srmPValue: 1 }));
    vi.stubGlobal("fetch", fetchMock);
    const { Wrapper } = createWrapper();
    const { result } = renderHook(() => useAbExperimentExposureQuery(null, query), { wrapper: Wrapper });
  ...
  it("posts manifestKey + dateRange when enabled", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const { Wrapper } = createWrapper();
    const { result } = renderHook(() => useAbExperimentExposureQuery("/en/about", query), { wrapper: Wrapper });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
