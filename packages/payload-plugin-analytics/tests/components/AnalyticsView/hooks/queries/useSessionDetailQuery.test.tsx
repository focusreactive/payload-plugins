...
describe("useSessionDetailQuery", () => {
  it("does not fetch when sessionId is null", async () => {
    ...
    const { result } = renderHook(() => useSessionDetailQuery(null, { preset: "last-7d" }), { wrapper: Wrapper });
  ...
  it("fetches /api/analytics/sessions/<id> when an id is provided", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const { Wrapper } = createWrapper();
    const { result } = renderHook(() => useSessionDetailQuery("sess-1", { preset: "last-7d" }), { wrapper: Wrapper });
    ...
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toEqual({ dateRange: { preset: "last-7d" } });
  });
});
