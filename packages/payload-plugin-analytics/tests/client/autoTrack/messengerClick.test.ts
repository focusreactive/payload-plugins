...
describe("installWhatsappClick", () => {
  it.each(["https://wa.me/1234", "https://api.whatsapp.com/send?phone=1"])("fires lead_action with fr_lead_type=whatsapp_click on %s", (href) => {
    const provider = makeProvider();
    const cleanup = installWhatsappClick(provider);
    document.body.innerHTML = `<a id="a" href="${href}">x</a>`;
    document.querySelector("#a")!.click();
    expect(provider.trackEvent).toHaveBeenCalledWith("lead_action", expect.objectContaining({ fr_lead_type: "whatsapp_click" }));
    cleanup();
  });
...
describe("installTelegramClick", () => {
  it.each(["https://t.me/foo", "https://telegram.me/bar"])("fires lead_action with fr_lead_type=telegram_click on %s", (href) => {
    const provider = makeProvider();
    const cleanup = installTelegramClick(provider);
    document.body.innerHTML = `<a id="a" href="${href}">x</a>`;
    document.querySelector("#a")!.click();
    expect(provider.trackEvent).toHaveBeenCalledWith("lead_action", expect.objectContaining({ fr_lead_type: "telegram_click" }));
    cleanup();
  });
