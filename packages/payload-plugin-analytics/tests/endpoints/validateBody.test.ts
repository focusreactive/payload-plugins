...
import { AnalyticsQuerySchema, TopNQuerySchema, SessionsListQuerySchema, TopCountriesQuerySchema } from "../../src/endpoints/validateBody";
...
describe("AnalyticsQuerySchema", () => {
  ...
  it("rejects from > to", () => {
    ...
    if (!r.success) expect(r.error.issues.some((i) => /from must be on or before to/iu.test(i.message))).toBe(true);
  });
