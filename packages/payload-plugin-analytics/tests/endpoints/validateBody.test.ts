...
describe("AnalyticsQuerySchema", () => {
  ...
  it("accepts preset 'last-14d' (default)", () => {
    expect(AnalyticsQuerySchema.safeParse({ dateRange: { preset: "last-14d" } }).success).toBe(
      true
    );
  });
  it("accepts custom dateRange", () => {
    expect(
      AnalyticsQuerySchema.safeParse({ dateRange: { from: "2026-01-01", to: "2026-01-31" } })
        .success
    ).toBe(true);
  });
  ...
  it("rejects from > to", () => {
    ...
    if (!r.success)
      expect(r.error.issues.some((i) => /from must be on or before to/iu.test(i.message))).toBe(
        true
      );
  });
  it("rejects non-ISO date strings", () => {
    expect(
      AnalyticsQuerySchema.safeParse({ dateRange: { from: "01/01/2026", to: "01/31/2026" } })
        .success
    ).toBe(false);
  });
...
describe("TopNQuerySchema", () => {
  ...
  it("clamps limit > 100", () => {
    expect(
      TopNQuerySchema.safeParse({ dateRange: { preset: "last-7d" }, limit: 101 }).success
    ).toBe(false);
  });
  it("rejects limit < 1", () => {
    expect(TopNQuerySchema.safeParse({ dateRange: { preset: "last-7d" }, limit: 0 }).success).toBe(
      false
    );
  });
});
...
describe("TopCountriesQuerySchema", () => {
  ...
  it("accepts dimension 'country'", () => {
    const r = TopCountriesQuerySchema.safeParse({
      dateRange: { preset: "last-7d" },
      dimension: "country",
    });
...
describe("SessionsListQuerySchema", () => {
  it("defaults limit to 50, accepts hadLeadAction", () => {
    const r = SessionsListQuerySchema.parse({
      dateRange: { preset: "last-7d" },
      hadLeadAction: true,
    });
  ...
  it("rejects cursor that is not a string", () => {
    expect(
      SessionsListQuerySchema.safeParse({ dateRange: { preset: "last-7d" }, cursor: 123 }).success
    ).toBe(false);
  });
  it("rejects limit > 250", () => {
    expect(
      SessionsListQuerySchema.safeParse({ dateRange: { preset: "last-7d" }, limit: 251 }).success
    ).toBe(false);
  });
...
type _ChkSessionList =
  z.infer<typeof SessionsListQuerySchema> extends SessionsListQuery ? true : false;
type _ChkTopCountries =
  z.infer<typeof TopCountriesQuerySchema> extends TopCountriesQuery ? true : false;
