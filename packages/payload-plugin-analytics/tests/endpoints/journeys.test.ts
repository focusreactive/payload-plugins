...
describe("POST /api/analytics/journeys", () => {
  ...
  it("returns 200 with rows + sessionsConsidered + truncated when valid", async () => {
    ...
    expect(typeof json.sessionsConsidered).toBe("number");
    expect(typeof json.truncated).toBe("boolean");
  });

  it("returns 200 with setupRequired+missing on customEvent:fr_session_id INVALID_ARGUMENT", async () => {
