...
export default function CarouselCard({ image, text, isActive, effect }: ICarouselCardProps) {
  ...
  return (
    <div
      className={cn("user-select-none border-secondaryColor bg-bgColor h-full border p-6 transition-all duration-300", {
    ...
    >
      ...
      {text && <RichText {...text} className={"prose-headings:mb-1 prose-headings:text-xl prose-headings:font-medium prose-p:text-base"} />}
    </div>
  );
}
