...
export function IconButton({ className, title, children, variant, size, isActive, tabIndex, onClick, ...nativeButtonProps }: Props) {
  return (
    <button className={cn(variants({ variant, size, isActive }), className)} type="button" title={title} aria-label={title} onClick={onClick} tabIndex={tabIndex} {...nativeButtonProps}>
      {children}
    </button>
  );
}
