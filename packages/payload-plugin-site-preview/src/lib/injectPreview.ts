...
const frameTags = (api: string, scrollOffset = "") => {
...
export const injectPreview = (
  html: string,
  { base, api, scrollOffset }: { base: string; api: string; scrollOffset?: string }
) => {
  ...
  return end < 0
    ? withBase
    : `${withBase.slice(0, end)}${frameTags(api, scrollOffset)}\n${withBase.slice(end)}`;
};
