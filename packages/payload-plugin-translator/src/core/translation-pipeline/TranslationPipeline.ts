...
export class TranslationPipeline {
  ...
  async execute(config: PipelineConfig): Promise<PipelineResult | null> {
    ...
    for (const stage of this.stages) {
      ...
      const nothingWasSelected = ctx.fieldChunks?.length === 0;
      if (nothingWasSelected) {
        return ctx.hasCarriedValues && ctx.filteredData
          ? { translatedData: ctx.filteredData }
          : null;
      }
      // Selected leaves already hold raw source in `filteredData` — returning it would persist
