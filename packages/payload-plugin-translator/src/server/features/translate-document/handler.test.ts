...
describe("TranslateDocumentHandler", () => {
  ...
  beforeEach(() => {
    ...
    handler = new TranslateDocumentHandler(
      mockTranslationProvider,
      mockSchemaMap
    );
  });
  ...
  describe("success responses", () => {
    ...
    it("returns success after saving translated document", async () => {
      const { TranslationPipeline } =
        await import("../../modules/translation-pipeline");
      (
        TranslationPipeline as unknown as ReturnType<typeof vi.fn>
      ).mockImplementation(() => ({
        execute: vi
          .fn()
          .mockResolvedValue({
  ...
  describe("saving translated documents", () => {
    beforeEach(async () => {
      ...
      (
        ...
        execute: vi
          .fn()
          .mockResolvedValue({ translatedData: { title: "Translated" } }),
      }));
    });
    ...
    it("sets _status to draft when versions with drafts enabled", async () => {
      (
        mockPayload.collections["posts"].config as { versions: unknown }
