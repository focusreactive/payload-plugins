...
describe("OverwriteStrategy", () => {
  ...
  describe("shouldTranslate", () => {
    ...
    it("returns true when sourceValue is non-empty object", () => {
      expect(
        strategy.shouldTranslate({ sourceValue: { key: "value" }, targetValue: undefined })
      ).toBe(true);
    });

    it("returns true when sourceValue is non-empty array", () => {
      expect(strategy.shouldTranslate({ sourceValue: [1, 2, 3], targetValue: undefined })).toBe(
        true
      );
    });
    ...
    it("returns false when sourceValue is null or undefined", () => {
      ...
      expect(strategy.shouldTranslate({ sourceValue: undefined, targetValue: "existing" })).toBe(
        false
      );
    });
    ...
    it("ignores targetValue completely", () => {
      ...
      expect(
        strategy.shouldTranslate({ sourceValue: "hello", targetValue: "already translated" })
      ).toBe(true);
