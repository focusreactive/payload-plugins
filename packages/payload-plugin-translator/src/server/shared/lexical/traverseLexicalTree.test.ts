...
describe('traverseLexicalTree', () => {
  describe('traversal order', () => {
    it('visits all nodes depth-first', () => {
      ...
      expect(visited).toEqual(['root', 'paragraph', 'text', 'text', 'paragraph', 'text'])
    })
  ...
  describe('edge cases', () => {
    it('handles nodes without children property', () => {
      const visited: string[] = []
      const node = { type: 'text', text: 'hello' } as any
    ...
    it('handles deeply nested structures', () => {
      const visited: string[] = []
      const root = createNode('root', [
        createNode('list', [createNode('listitem', [createNode('paragraph', [createNode('text')])])]),
      ])
      ...
      expect(visited).toEqual(['root', 'list', 'listitem', 'paragraph', 'text'])
    })
