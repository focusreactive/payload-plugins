...
describe('ServerResponse', () => {
  ...
  describe('client error responses', () => {
    ...
    it('badRequest includes details when provided', async () => {
      const response = ServerResponse.badRequest('Validation failed', { fields: ['name'] })
      expect(response.status).toBe(400)
    ...
    it('conflict returns 409', async () => {
      ...
      expect(await response.json()).toEqual({ message: 'Resource already exists' })
    })

    it('unprocessableEntity returns 422', async () => {
      const response = ServerResponse.unprocessableEntity('Cannot process', { reason: 'invalid' })
  ...
  describe('validation helpers', () => {
    ...
    it('invalidInput returns 400 with field name', async () => {
      const response = ServerResponse.invalidInput('email', 'Must be a valid email')
      expect(response.status).toBe(400)
      expect(await response.json()).toEqual({
  ...
  describe('custom response', () => {
    ...
    it('custom includes data when provided', async () => {
      const response = ServerResponse.custom('With data', 200, { extra: 'info' })
      expect(response.status).toBe(200)
      expect(await response.json()).toEqual({
        message: 'With data',
        data: { extra: 'info' },
