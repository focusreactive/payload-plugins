...
export const buildDuplicateVariantHandler =
  <TVariantData extends object>(pluginConfig: AbTestingPluginConfig<TVariantData>): PayloadHandler =>
  async (req) => {
    if (!req.payload) {
    ...
    const abConfig = pluginConfig.collections[collectionSlug];
    if (!abConfig) {
      return Response.json(
        { error: `Collection "${collectionSlug}" is not configured for A/B testing` },
        { status: 400 },
      );
    }
    ...
    try {
      for (const variant of existingVariants) {
        await req.payload.db.updateOne({
          ...
          id: variant.id as string | number,
      ...
      await recomputeManifestForParent(docId, collectionSlug, abConfig, pluginConfig, req);
