...
export function vercelEdgeAdapter<TVariantData extends object>(
  ...
  const missing = (["configID", "configURL", "vercelRestAPIAccessToken"] as const).filter(
    (key) => !config[key]
  );
  if (missing.length > 0) {
    console.warn(
      `[${PLUGIN_NAME}] vercelEdgeAdapter: missing ${missing.join(", ")} (likely EDGE_CONFIG_ID / EDGE_CONFIG / VERCEL_REST_API_ACCESS_TOKEN not set). Edge Config reads/writes will fail.`
    );
  }
